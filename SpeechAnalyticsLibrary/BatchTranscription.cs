...
namespace SpeechAnalyticsLibrary
{
    public class BatchTranscription
    {
        ...
        public async Task<TranscriptionResponse?> StartBatchTranscription(string foundryResourceName, string sourceContainerUrl, string destinationContainerUrl, Uri? blobFile = null)
        {
            try
            {


                var transcriptionEndpoint = $"https://{foundryResourceName}.cognitiveservices.azure.com/speechtotext/transcriptions:submit?api-version=2024-11-15";
                ...
                var transcriptionReq = new Transcription()
                {
                    ...
                    Properties = new TranscriptionProperties()
                    {
                        ...
                        {
                            CandidateLocales = new[] { new CultureInfo("en-US"), new CultureInfo("es-ES") }
                        },
                ...
                var transcrReqJson = JsonSerializer.Serialize(transcriptionReq, serializerOptions);
                ...
                using HttpRequestMessage request = new HttpRequestMessage();
                {
                    ...
                    request.RequestUri = new Uri(transcriptionEndpoint);
        ...
        public async Task<TranscriptionResponse?> StartBatchTranscription(string foundryResourceName, string sourceContainerUrl, string destinationContainerUrl, FileInfo? localFile = null)
        {
            try
            {
                _ = destinationContainerUrl;
                ...
                return await StartBatchTranscription(foundryResourceName, sourceContainerUrl, destinationContainerUrl, blobFile);
            }
            catch (Exception ex)
            {
                ...
                try
                {
                    using HttpRequestMessage request = new HttpRequestMessage();
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            var speakers = await agentClient.GetSpeakerNames(tmpSource, output.RawTranscriptionText);
                            if (speakers != null)
                            {
                                var tmp = output.RawTranscriptionText;
                            ...
                            {
                                transcriptions.Add((tmpSource, output.RawTranscriptionText));
                            }
                        }
                        else
                        {
