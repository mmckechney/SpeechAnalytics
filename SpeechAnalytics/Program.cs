...
namespace SpeechAnalytics
{
    public class Program
    {
        ...
        public static void SetUp(string[] args)
        {
            ...
            identityHelper = new IdentityHelper(logFactory.CreateLogger<IdentityHelper>(), settings);
                    ...
                    foreach (var transcription in transcriptions)
                    {
                        try
                        {
                            ...
                            if (rerun)
                            {
                                ...
                                {
                                    log.LogError($"[CorrelationId: {correlationId}] Error getting AI Insights: {ex.Message}", new Dictionary<string, object> { { "Source", transcription.source } });
                                    Console.WriteLine("An error occurred while analyzing the transcript. Please try again later.");
                                    continue;
                                }
                                if (string.IsNullOrWhiteSpace(insights))
                                {
                                    log.LogError($"[CorrelationId: {correlationId}] Failed to get AI Insights", new Dictionary<string, object> { { "Source", transcription.source } });
        ...
        private static async Task<List<(string source, string transcription)>> NewTranscriptions(string? path)
        {
            ...
            var foundrySettings = settings.FoundryAgent;
            ...
            log.LogInformation("");
            var initialResponse = await batch.StartBatchTranscription(foundrySettings.ResourceName, settings.Storage.SourceContainerUrl, settings.Storage.TargetContainerUrl, fileInfo);
        ...
        private static LogLevel SetLogLevel(string[] args)
        {
            ...
            {
                Program.logLevel = LogLevel.Debug;
                return LogLevel.Debug;
            }
            if (args.Contains("--trace"))
            {
                Program.logLevel = LogLevel.Trace;
                return LogLevel.Trace;
            }
            if (args.Contains("--info"))
            {
                Program.logLevel = LogLevel.Information;
                return LogLevel.Information;
            }
            if (args.Contains("--warn") || args.Contains("--warning"))
            {
            ...
            {
                Program.logLevel = LogLevel.Error;
